...
def create_app() -> FastAPI:
    app = FastAPI(title="F2 Downloader Web", version="0.1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    async def put_settings(payload: DownloaderSettings) -> DownloaderSettings:
        try:
            ...
            normalized.download_path = download_path_policy.normalize(payload.download_path)
    ...
    async def post_task(payload: TaskCreateRequest) -> TaskSummary:
        ...
        try:
            normalized_download_path = download_path_policy.normalize(settings.download_path)
